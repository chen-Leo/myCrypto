...
func DesEnc(plainText, key, iv []byte, mode modeWay, padding paddingWay) ([]byte, error) {
...
func DesDec(cipherText, key, iv []byte, mode modeWay, padding paddingWay) ([]byte, error) {
	block, err := des.NewCipher(key)
	if err != nil {
		return nil, err
	}
